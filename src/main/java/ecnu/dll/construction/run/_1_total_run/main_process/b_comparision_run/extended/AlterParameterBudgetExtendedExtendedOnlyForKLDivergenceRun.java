...
//        String inputDataPath = Constant.DEFAULT_INPUT_PATH;
        ...
        int arraySize = Constant.ALTER_PRIVACY_BUDGET_ARRAY_for_DAM_and_SubsetGeoI_Comparison_for_KL_Divergence.length;
        /*
            1. 设置cell大小的参数（同时也是设置整数input的边长大小）
         */
        double inputLengthSize = Constant.DEFAULT_SIDE_LENGTH_NUMBER_SIZE_for_DAM_and_SubsetGeoI_Comparison_for_KL_Divergence;
        double gridLength = inputSideLength / inputLengthSize;

        /*
            2. 设置隐私预算budget的变化数组
         */
        double[] epsilonArray = Constant.ALTER_PRIVACY_BUDGET_ARRAY_for_DAM_and_SubsetGeoI_Comparison_for_KL_Divergence;
        ...
        for (int i = 0; i < arraySize; i++) {
            ...
            // for DAM
            tempDiskExperimentResult = DAMRun.runWithoutWassersteinDistance(integerPointList, rawDataStatistic, gridLength, inputSideLength, alterDiskOptimalSizeB[i]*gridLength, epsilonArray[i], kParameter, xBound, yBound);
            diskExperimentResultList.add(tempDiskExperimentResult);


            // for Subset-Geo-I-norm2
            tempLocalPrivacy = Initialized.damELPKLTable.getLowerBoundLocalPrivacyByEpsilon(inputLengthSize, epsilonArray[i]);
            transformedEpsilon = Initialized.subGeoIELPKLTable.getEpsilonByUpperBoundLocalPrivacy(inputLengthSize, tempLocalPrivacy);
            tempSubsetGeoITwoNormExperimentResult = SubsetGeoITwoNormRun.runWithoutWassersteinDistance(integerPointList, rawDataStatistic, gridLength, inputSideLength, transformedEpsilon, xBound, yBound);
